<template>
  <div v-if="!load">
    <div class="table-responsive">
      <table class="table">
        <thead>
        <tr>
          <th scope="col">#</th>
          <th scope="col">Название</th>
          <th scope="col" style="padding-right: 57px;">Изображение</th>
          <th scope="col">Цена</th>
          <th scope="col">Действие</th>
        </tr>
        </thead>
        <tbody>
        <tr v-for="item in items" :key="item.id">
          <th scope="row">{{ item.id }}</th>
          <td>{{ item.title }}</td>
          <td><img width="100" :src="img_URL+'storage/uploads/'+item.image" alt=""></td>
          <td >{{ item.price }} руб.</td>
          <td nowrap>
            <nuxt-link :to="{name: 'admin.pharmacy.edit', params: {id: item.id}}">
              <b-icon class="mr-3" title="редактировать" variant="warning" icon="pencil" scale="1.2"></b-icon>
            </nuxt-link>
            <nuxt-link :to="{name: 'product.single', params: {slug: item.slug, id: item.id}}">
              <b-icon class="mr-3" title="на странице" variant="success" icon="door-open" scale="1.2"></b-icon>
            </nuxt-link>
            <b-icon style="cursor: pointer" @click.prevent="deleteProduct(item.id)" class="mr-3" title="удалить из бд" variant="danger" icon="calendar-x-fill" scale="1.2"></b-icon>


          </td>
        </tr>

        </tbody>
      </table>
    </div>
  </div>
  <div v-else class="wrap-spin">
    <b-icon icon="arrow-clockwise" animation="spin-pulse" font-scale="4"></b-icon>

  </div>
</template>
<script>
    export default {
        layout: 'admin',
        data: () => ({
            items: [],
            img_URL: process.env.img_URL,
            load: true
        }),
        methods: {
            deleteProduct(id) {
                if (confirm('Удалить продукт?')) {
                    this.load = true
                    this.$axios.delete(`/product/${id}`)
                        .then(res => {
                            const data = this.$axios.get('/products').then(({data}) => {
                                let item = [];
                                this.items = data.data
                                this.load = false
                            }).catch(res => {
                                console.log(res.response)
                                this.load = false
                            }).finally(() => {
                                this.load = false
                            })
                        })
                        .catch((res) => {
                            console.log(res.response)
                        })
                }

            }
        },
        created() {

                const data = this.$axios.get('/products').then(({data}) => {
                    let item = [];
                    this.items = data.data
                    this.load = false
                }).catch(res => {
                    console.log(res.response)
                    this.load = false
                })


        }
    }
</script>
<style>
  table th {
    vertical-align: middle!important;
  }
</style>
